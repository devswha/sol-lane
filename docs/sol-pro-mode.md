...
1. ~~`lane serve` — SSE + usage 계약을 만족하는 shim~~ — 실제 엔진에 연결돼 돈다.
   ...
   쓰기 전에 취소한다.
2. ~~동시 요청 직렬화 (브라우저 1개 전제)~~ — 브라우저 프로필에 건 OS 락이다.
   프로세스 안의 `threading.Lock`은 다른 터미널에서 뜬 두 번째 lane을 막지 못했다.
3. **tool call 브리지** — 프롬프트 주입 + 파싱, 파싱 실패는 fail-closed.
   지금은 브리지가 없다는 사실을 숨기지 않는 지점까지다: 비어있지 않은 `tools`,
   tool 결과, 텍스트가 아닌 파트는 조용히 버려지는 대신 400으로 거절된다.
4. **세션↔ChatGPT 대화 매핑** — 엔진 v0.6.0이 전송 한 판을 대화 URL에 결속하고
   `--harvest`로 되짚을 수 있게 해줬지만, 그건 `lane review`/`lane harvest` 경로다.
   `serve`는 여전히 호출마다 전체 트랜스크립트를 프롬프트로 다시 렌더한다 —
   `render_prompt(messages)`가 그 증거다. gjc 세션 하나를 대화 하나에 묶고 증분만
   보내는 일은 남아 있다.

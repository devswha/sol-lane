...
## omg DOM 이식 (2026-08-27)

oh-my-gajae-code의 `eaab0d8`(2026-08-19, Chrome 145 + 2026-08 모델 메뉴 실측)에서
우리 구조에 해당하는 세 가지를 이식했다: `clear_composer`의 OS별 select-all(Meta+a는
Linux에서 Super 키라 전체선택이 아니었다), `put_text`의 실제 클릭 진입 + 잔여 draft
선행 제거, 추론단계 클릭의 role locator 우선 경로(React 재마운트로 ElementHandle이
떨어지는 대응). 이식하지 않은 것: CDP↔프로필 argv 결속 검증(영수증 소실 대응) —
공개 배포용 위협 모델(남의 브라우저에 우연히 붙는 것 방지)이라 단일 운영자 도구인
이 레인엔 해당 경로가 없다. 라디오→슬라이더 aria-checked 반전 검증은 이미 재오픈
after 검증으로 동등하게 갖추고 있어 미이식.

## 2026-08-27 실측 — effort UI 재적응 (5판)

라이브 판 5회로 2026-08-27 UI를 실측했다. 이 UI는 (1) 라디오 목록이 없고,
(2) '모델|GPT-5.6 Sol', '추론 수준|최대' 값-줄 menuitem으로 렌더되며, (3) 성능
슬라이더(role=slider, 0..4)는 키보드(Arrow/focus/End/Home)에 반응하지 않고,
(4) Escape 후 메뉴 재오픈이 빈 포털을 준다(재마운트). 고친 것 세 가지:

- **별칭 매칭**: `EFFORT_ALIASES`(pro≡최대≡울트라≡ultra≡max 등)로 라벨을
  정확 매칭한다. 'pro' 부분문자열 매칭은 로케일 라벨('최대')을 영원히 못 찾았고,
  반대로 모델명의 'Pro'를 오인할 위험도 있었다(omg eaab0d8과 동일 처방).
- **이미 선택됨 조작 생략**: 메뉴 읽기가 목표 단계를 가리키면(이미 '최대') 슬라이더를
  억지로 밀지 않고 그대로 인정한다. 조작 불가능한 슬라이더를 밀다 전송을 놓치는
  것보다 낫다.
- **already 경로의 before 검증**: 재오픈이 빈 메뉴를 줘도, 조작이 없었던 경로는
  최초 읽기(before)로 검증한다 — before==after가 논리적으로 보장되기 때문.
  조작했던 경로가 빈 메뉴를 받으면 여전히 실패(fail-closed 유지).

전 과정에서 증거 영속화가 처음으로 실전 가동됐다: 4번의 실패가 각각
`.insane-review/failed_*.log`로 남아 진단이 콘솔 스크롤백이 아니라 디스크에서
일어났다. 실패 판 모두 전송 전에 중단(구독 메시지 낭비 0통).

## 2026-08-31 실측 — 같은 날 두 번째 UI 변화 (3판 실패 + 2무료프로브 → 완주)

omg 연동 실측 중 모델 메뉴가 또 바뀌어 있었다. (1) 모델명이 'GPT-5.6 Sol'에서
**'5.6 Sol'로 접두사 없이** 표시되고, (2) effort 표시가 메뉴에서 사라져 **composer pill의
둘째 줄에만** 남았으며, (3) 메뉴가 열려 있는 동안 pill이 DOM에서 사라진다. 세 가지 고침:
`MODEL_NAME_RE`에 접두사 없는 버전 숫자 추가(기본/즉시/중간/높음/최대엔 숫자가 없어
구분 유지), `_model_name_matches`의 양방향 GPT- 접두사 제거 매칭, select_model이
**메뉴를 열기 전에** pill을 스냅샷해 already-pill 경로로 인정. 셋 다 무료 DOM 프로브로
원인을 확정한 뒤 적용했고, 마지막 판은 omg 소독 워크트리에서 완주(5.6 Sol, 응답 저장).

이날 하루에 UI가 두 번 바뀌었다는 것이 본질적인 정보다: DOM 계약은 주 단위로 깨지며,
무료 프로브(전송 없는 메뉴 관찰)가 수리 사이클의 표준 도구다. 실패 3판 모두 전송 전
fail-closed(메시지 낭비 0통), 증거는 `failed_20260831_*.log` 3건.

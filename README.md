...
`review.py`의 `PACK_WARN_BYTES = 200_000` 주석은 "~100 KB 팩은 2분에 답한다"고 적혀
있는데 실측과 맞지 않는다. 100 KB 미만도 18분을 쓴다. 경고선(195 KB)은 이미 실패한
크기(164 KB)보다 위에 있다.

지금 확실한 건 이것뿐이다: **크기는 실패 확률과 같이 간다.** 중단은 31분·29분 구간에서
관측됐고, 20분 미만 판에서는 아직 관측되지 않았다. 그래서 자기감사는 한 판에 전부
넣지 말고 파일 집합을 쪼개는 쪽이 맞다.

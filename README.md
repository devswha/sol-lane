...
팩을 반으로 줄여도 추론 시간은 줄지 않는다 — 질문의 난이도가 시간을 정한다. 죽는 건
크기가 아니라 **30~40분 구간**이다.

그래서 `force_answer_after = 1200`이 기본이다. 스트림이 아직 살아 있는 20분 지점에서
'지금 답변 받기'를 눌러 답을 받아낸다. 잘린 답은 완전한 답보다 못하지만, 비교 대상은
완전한 답이 아니라 **아무것도 없음**이다(5판 연속).

`review.py`의 `PACK_WARN_BYTES = 200_000`은 이 곡선과 맞지 않는다. 경고선(195 KB)은
이미 죽은 크기(164 KB)보다 위에 있고, 78 KB도 죽었다. 크기 경고만으로는 이 실패를
막을 수 없다 — 시간을 묶는 쪽이 실제 방어다.

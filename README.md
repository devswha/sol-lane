...
| 팩 | 추론 | 결과 |
|---|---|---|
| 70 KB (7파일) | 21분+ | 진행 중 (force 클릭은 발화하지 않았다) |
| 78 KB (7파일) | 37분 00초 | 스스로 중단, assistant 노드 0개 |
| 93 KB | 18분+ | 옛 20분 마감에 잘려 유실 |
| 164 KB (22파일) | 31분 33초 | 스스로 중단, assistant 노드 0개 |
| 290 KB | 40분 20초 | 엔진 회수 실패, 수동 salvage |

처음엔 크기 문제로 보였다. 아니다. **78 KB 판이 164 KB 판보다 6분 더 추론하고 죽었다.**
시간을 정하는 건 팩 크기가 아니라 질문의 난이도다. 죽는 구간은 30~40분이다.

**그리고 시간을 묶을 방법이 지금은 없다.** 엔진 v0.5.8+는 `force_answer_after` 지점에서
'지금 답변 받기'를 클릭하지만, 2026-08-13 실측으로 그 어포던스가 UI에 존재하지 않는다:

```
div[data-testid="cot-v5-pinned-row"]  → 0개
텍스트 /답변\s*받기|Get answer|answer now/ 매칭 → 0개
data-testid 중 cot|pinned|thinking|reason → 없음
```

스트리밍이 살아 있는 시점(21분)에 확인한 값이다. 클릭 실패는 조용해서(성공 시에만
로그) 6회 재시도가 전부 헛돌고 로그에는 `⏳ 생성중`만 남는다. 그래서
`force_answer_after = 1200`은 **현재 UI에서 무효**다 — UI가 버튼을 되돌리면 그때부터
동작하도록 값은 남겨둔다.

남은 실질 방어는 두 개뿐이다.

1. **질문을 작게 쪼갠다.** 30분 밑에서 끝나도록. 팩을 줄이는 게 아니라 요구를 줄인다.
2. **죽어도 건진다.** 결속 URL은 전송 직후 manifest에 남고(`0003` 패치), `lane harvest`가
   메시지 없이 재회수하며, 답이 없고 추론만 남았으면 salvage가 그 텍스트를 가져온다.
   2026-08-13의 두 판이 그 경로로 살아났다 — 중단된 추론 패널에서 실제 결함 2건이 나왔다.

`review.py`의 `PACK_WARN_BYTES = 200_000`은 이 곡선과 맞지 않는다. 78 KB도 죽었으니
크기 경고는 유출량 힌트일 뿐 이 실패의 방어가 아니다.
